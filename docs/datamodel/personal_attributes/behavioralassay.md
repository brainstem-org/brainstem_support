...
| ``Setup type`` | The standardized apparatus type (e.g., "Elevated Plus Maze", "Open Field Arena"). Each setup type has a category describing the experimental condition (Freely Moving Awake, Head-Fixed Awake, etc.) (**required**). Must reference an existing [setup type]({{"datamodel/taxonomies/setuptype/"|absolute_url}}). |
